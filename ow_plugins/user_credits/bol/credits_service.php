...
final class USERCREDITS_BOL_CreditsService
{
    ...
    /**
     * Site stat entity buy
     */
    CONST SITE_STAT_ENTITY_BUY = 'usercredits_buy';

    /**
     * Site stat entity spent
     */
    CONST SITE_STAT_ENTITY_SPENT = 'usercredits_spent';
    ...
    public function trackAction( $pluginKey, $action, $userId, $checkInterval = true, $extra = null )
    {
        ...
        if ( $defaults['status'] )
        {
            $groupKey = !empty($extra['actionsGroupKey']) ? $extra['actionsGroupKey'] : null;
            $this->logAction($actionDto->id, $userId, $actionPrice->amount, null, $groupKey);
        }
     ...
     * @return bool
     */
    public function logAction( $actionId, $userId, $amount, $additionalParams = null, $groupKey = null )
    {
        if (!$userId) {
            return false;
        }
        ...
        $log->groupKey = $groupKey;

        $this->logDao->save($log);

        $actionDto = USERCREDITS_BOL_CreditsService::getInstance()->findActionById($actionId);
        $statService = BOL_SiteStatisticService::getInstance();

        // spent credits
        if ( $amount < 0 )
        {
            $statService->addEntity(self::SITE_STAT_ENTITY_SPENT, $actionId, abs($amount));
        }
        else if ( $actionDto->pluginKey == 'usercredits' && $actionDto->actionKey == 'buy_credits' )
        {
            // buy
            $statService->addEntity(self::SITE_STAT_ENTITY_BUY, $actionId, $amount);
        }
    ...
    {
        return (int) $this->packDao->findPacksHistoryCount();
    }
    ...
    {
        return $this->packDao->findPacksHistory($page, $limit);
    }
